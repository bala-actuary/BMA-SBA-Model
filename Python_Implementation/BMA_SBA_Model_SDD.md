...
*   **Technology:** Python, using libraries like NumPy and pandas for data manipulation.
...
*   **Description:** A service that manages a library of both prescribed and non-prescribed scenarios. It will expose an API for adding new scenarios as plugins, primarily through a formal "Scenario Manifest" structure. Each plugin will consist of a folder containing:
    ...
    *   **`logic.py` (Optional):** A Python script for scenarios that require complex, programmatic logic beyond simple parameter shifts. This script will expose a well-defined entry point function for the scenario engine to call.
*   **Technology:** Python with a Flask or FastAPI framework for the API. Scenario definitions (including YAML and Python logic) will be version-controlled and stored in the database.
...
*   **Technology:** Python with libraries like `scipy.stats` for statistical modeling. For performance, we may use a distributed computing framework like Dask or Spark.
...
*   **Technology:** A web service built with Python (Flask/FastAPI) that handles file uploads and data parsing. Data validation rules will be rigorously implemented using machine-readable data schemas (e.g., JSON Schema) in conjunction with libraries like Cerberus or Pydantic.
*   **API:** A RESTful API for uploading data files and getting validation results.

### 3.5. Cash Flow Projection Module

*   **Description:** A service that takes scenario and company data as input and projects asset and liability cash flows.
*   **Technology:** Python with heavy use of NumPy and pandas for high-performance calculations.
...
*   **Technology:** Python. Report generation will use libraries like `Jinja2` for templating and `WeasyPrint` for PDF generation.
*   **API:** A RESTful API for initiating assessments and retrieving reports.

### 3.7. User Interface (Web-based dashboard)

*   **Description:** A single-page application (SPA) that provides the user interface for the BMA analysts.
...
Data models for key entities like `Scenario`, `Submission`, and `ModelRun` will be defined in the application code, likely using an Object-Relational Mapping (ORM) library like SQLAlchemy for the Python services.
